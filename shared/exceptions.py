...
class RiskLimitExceededError(RiskManagementError):
    ...
    def __init__(self, message: str, current_value: float, limit: float, **kwargs):
        ...
        kwargs.setdefault('error_code', 'RISK_LIMIT_EXCEEDED')
...
def handle_error(
    ...
    if isinstance(error, BaseTradingError):
        ...
        trading_error = TradingSystemError(
            message=str(error),
            details={'original_error_type': type(error).__name__, 'context': context},
            cause=error
        )
    ...
    return response if response else (default_response or {
        "success": False,
        "error": {
            "code": "UNKNOWN_ERROR",
